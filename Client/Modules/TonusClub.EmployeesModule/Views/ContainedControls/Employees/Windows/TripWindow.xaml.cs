...
namespace TonusClub.EmployeesModule.Views.ContainedControls.Employees.Windows
{
    public partial class TripWindow
    {
        ...
        private void AssetButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (emps.Length == 0)
            {
                TonusWindow.Alert("Ошибка", "Необходимо выбрать хотя бы одного сотрудника!");
                return;
            }
            if (EndDate < BeginDate)
            {
                TonusWindow.Alert("Ошибка", "Дата окончания не может быть ранее даты начала!");
                return;
            }
            if (String.IsNullOrEmpty(Destination) || String.IsNullOrEmpty(Base) || String.IsNullOrEmpty(Target))
            {
                TonusWindow.Alert("Ошибка", "Необходимо заполнить все поля!");
                return;
            }
