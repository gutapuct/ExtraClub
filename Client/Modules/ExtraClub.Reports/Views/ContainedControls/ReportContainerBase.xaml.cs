...
using ExtraClub.Reports.Business;
...
namespace ExtraClub.Reports.Views.ContainedControls
{
    public partial class ReportContainerBase
    {
            ...
            get
            {
                ...
                try
                {
                    if (ResultTable.Columns.Contains("_customerId") ||
                        Type.GetType(ResultTable.ExtendedProperties["EntityType"] + ", ExtraClub.ServiceModel") == typeof(Customer))
                    {
        ...
        private void PdfButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ExtraWindow.Alert("Ошибка", "При экспорте отчета возникла ошибка:\n" + ex.Message);
            }
        }
        ...
        protected virtual void GenerateInternal()
        {
            ...
            catch (Exception ex)
            {
                Application.Current.Dispatcher.Invoke(new Action(() =>
                {
                    ExtraWindow.Alert("Ошибка", "При генерации отчета возникла ошибка:\n" + ex.Message);
                }));
            }
            finally
            {
        ...
        private void ResultGrid_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            if (IsRowClicked(e))
            {
                ...
                {
                    entityType = Type.GetType(row.Table.ExtendedProperties["EntityType"] + ", ExtraClub.ServiceModel");
                }
        ...
        private void SaveParamsClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                ExtraWindow.Alert("Ошибка", "Невозможно сохранить параметры, так как отчет их не содержит.");
                return;
            }
            ExtraWindow.Prompt("Сохранение параметров",
        ...
        private void TaskButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                entityType = Type.GetType(rows.First().Table.ExtendedProperties["EntityType"] + ", ExtraClub.ServiceModel");
            }
        ...
        {
            ExtraWindow.Confirm("Отправка отчета", "Отправить отчет франчайзору?\nПотребуется подключение к интернету.", wnd =>
            {
                if (wnd.DialogResult ?? false)
                {
                    ...
                    try
                    {
                        ...
                        ExtraWindow.Alert("Отчет отправлен", "Отчет отправлен успешно");
                    }
                    catch (EndpointNotFoundException ex)
                    {
                        ExtraWindow.Alert("Отправить отчет не удалось.", "Пожалуйста, проверьте подключение к интернету.\n" + ex.GetType().ToString() + "\n" + ex.Message);
                    }
                    catch (Exception ex)
                    {
                        ExtraWindow.Alert("К сожалению, отправить отчет не удалось.", "Ошибка при отправке отчета\n" + ex.GetType().ToString() + "\n" + ex.Message);
                    }
                }
            });
        }
