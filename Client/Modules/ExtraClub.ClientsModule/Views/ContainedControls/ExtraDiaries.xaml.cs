...
{
    public partial class ExtraDiaries : ModuleViewBase
    {
        ...
        {
            InitializeComponent();
            DataContextChanged += new DependencyPropertyChangedEventHandler(ExtraDiaries_DataContextChanged);
        }

        void ExtraDiaries_DataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            if(model != null)
            {
        ...
        private void ExportAnthroButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ExtraWindow.Alert("Отчет по клиенту", "Отчет по клиенту нельзя построить без измерений");
                return;
            }
            ExportToExcel();
        }
