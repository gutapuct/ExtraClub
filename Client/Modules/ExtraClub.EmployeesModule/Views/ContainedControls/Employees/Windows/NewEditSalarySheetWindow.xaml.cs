...
namespace ExtraClub.EmployeesModule.Views.ContainedControls.Employees.Windows
{
    ...
    public partial class NewEditSalarySheetWindow
    {
        ...
        {
            ExtraWindow.Alert("Отчет о формировании зарплаты", ((SalarySheetRow)((Button)sender).DataContext).Log);
        }
        ...
        private void PayClick(object sender, RoutedEventArgs e)
        {
                            ...
                            payClosed =>
                            {
                                if(payClosed.DialogResult ?? false)
                                {
                                    ...
                                    if(Decimal.TryParse(payClosed.TextResult, out amount))
                                    {
                                        ...
                                        {
                                            ExtraWindow.Alert("Ошибка", "Введенная сумма превышает выплаты по ведомости!");
                                        }
                                    }
                                    else
                                    {
                                        ExtraWindow.Alert("Ошибка", "Введенная сумма неверна!");
                                    }
                                }
                            }
                        );
            ...
            {
                ExtraWindow.Alert("Ошибка", "По данной ведомости сотруднику уже выплачена полная сумма!");
            }
        }
