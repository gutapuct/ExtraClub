...
namespace ExtraClub.ServerCore
{
    class ReportConstructorProcessor
    {
        ...
        public ReportConstructorProcessor(CustomReport report, Dictionary<string, object> parameters)
        {
            ...
            InitOnTable = true;
            context = new ExtraEntities();
            var user = UserManagement.GetUser(context);
            var baseType = Type.GetType(report.BaseTypeName + ", ExtraClub.ServiceModel");
        ...
        {
            var baseType = Type.GetType(reportInfo.BaseTypeName + ", ExtraClub.ServiceModel");
