...
namespace ExtraClub.ServerCore
{
    public static class OrganizerCore
    {
        ...
        {
            if(!new int[] { 0, 1, 2, 3 }.Contains(claim.ClaimTypeId)) throw new FaultException<string>("Необходимо обновление клиентского приложения ExtraClub Direction!", "Необходимо обновление клиентского приложения ExtraClub Direction!");
            using(var context = new ExtraEntities())
            {
                Claim dbClaim;
