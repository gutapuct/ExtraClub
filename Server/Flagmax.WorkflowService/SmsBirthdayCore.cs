...
namespace Flagmax.WorkflowService
{
    class SmsBirthdayCore
    {
                ...
                foreach (var item in data)
                {
                    ...
                    if (customer.HasEmail.HasValue && customer.HasEmail.Value && WorkflowCore.EmailValidate(item.Email))
                    {
                        context.EmailItems.AddObject(new EmailItem
                        {
                            ...
                            Message = "<p style='text-align:center'><img src='http://tonusclub.ru/media/67532/e-mail_1_3.jpg'></p>",
                            CreatedOn = DateTime.Now,
        ...
        public void SubtractionBonuses(TonusClub.ServerCore.CancelThreadInfo cancelInfo)
        {
            using (var context = new TonusEntities())
            {
