            ...
            using(var context = new TonusEntities())
            {
                ...
                foreach(var eId in visited)
                {
                    ...
                    if(ev.TicketId.HasValue)
                    {
                        ...
                        if(!new TonusEntities().UnitCharges.Any(i => i.EventId == ev.Id))
                        {
#if BEAUTINIKA
                            var uc = new UnitCharge
                            {
                                AuthorId = user.UserId,
                                Charge = ev.TreatmentConfig.IsMainTreatment ? (int)ev.TreatmentConfig.Price : 0,
                                ExtraCharge = ev.TreatmentConfig.IsMainTreatment ? 0 : (int)ev.TreatmentConfig.Price,
                            ...
                            BeautinikaCore.ChargeMaterials(eId);
                            context.UnitCharges.AddObject(uc);
#else
                            var x = new TonusEntities().ExecuteStoreCommand(@"
...
", user.UserId,//0
        ...
        private static void ValudateAvailableUnits(TonusEntities context, Ticket ticket, Guid configId)
        {
        ...
        public static TreatmentEvent PostNewTreatmentEvent(Guid ticketId, Guid treatmentId, DateTime visitStart)
        {
            using(var context = new TonusEntities())
            {
                ...
                var res = new TonusEntities().TreatmentEvents.Single(i => i.Id == te.Id);
                res.Init();
                return res;
            }
        }
