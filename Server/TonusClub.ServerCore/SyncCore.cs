...
using TonusClub.ServiceModel.Sync;
...
namespace TonusClub.ServerCore
{
    ...
    public static class SyncCore
    {
        ...
        public static void Syncronize()
        {
            ...
            if(new TonusEntities().Companies.Count() > 1)
            {
        ...
        public static void GetDataSetStreamEx(Guid companyId, long version, Func<Guid, long, DataSet> func, ref string log, int utcCorr, string path)
        {
            try
            {
                ...
                using(var master = new TonusClub.Entities.TonusEntities())
                using(var masterHelper = new TonusClub.Entities.TonusEntities())
                {
                ...
                foreach(DataTable dt in keys.Tables)
                {
                    ...
                    if(container.BaseEntitySets.Any(i => i.Name == tname && i.BuiltInTypeKind == BuiltInTypeKind.EntitySet))
                    {
                        for(int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            EntityKey key = new System.Data.EntityKey("TonusEntities." + tname, keyData);
                            object obj;
        ...
        public static long GetCurrentChangeTrackingVersion(TonusEntities context)
        {
            using(var conn = new SqlConnection(((EntityConnection)context.Connection).StoreConnection.ConnectionString))
            {
                conn.Open();
        ...
        public static long CommitStream(Stream stream, ref string log)
        {
            ...
            using(var local = new TonusClub.Entities.TonusEntities())
            {
                        ...
                        if(set != null)
                        {
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    EntityKey key = new System.Data.EntityKey("TonusEntities." + set.Name, keyData);
                                    object obj;
        ...
        internal static void UpdateObject(TonusEntities local, string setName, object obj, ref string log)
        {
            ...
            if(obj.GetValue(GetKeyNameByObjectType(obj.GetType())) != null && setName != null)
            {
                try
                {
                    ...
                    EntityKey key = new System.Data.EntityKey("TonusEntities." + setName, keyData);
                    object obja = null;
            ...
            //        local.AttachTo("TonusEntities." + setName, obj);
            //        if (objExists)
            //        {
            //            local.ObjectStateManager.ChangeObjectState(obj, EntityState.Modified);
            //        }
        ...
        private static void SyncUser(TonusEntities context, User user, ref string log)
        {
        ...
        public static void SyncronizeClaims()
        {
            using(var context = new TonusEntities())
            {
                var claimStatusIds = new[] { 0, 5, 7 };
                var claims = context.Claims.Where(i => claimStatusIds.Contains(i.StatusId)).ToArray();
                ChannelFactory<ISyncService> cf = new ChannelFactory<ISyncService>("SyncServiceEndpoint");
                var client = cf.CreateChannel();
                try
                {
                    foreach(var claim in claims.Where(i => i.StatusId == 0))
                    {
                        claim.FtmId = client.PostNewClaim(claim);
                        claim.StatusDescription = "Запрос размещен в FTM";
                        claim.StatusId = 2;
                        context.SaveChanges();
                    }

                    foreach(var claim in claims.Where(i => i.StatusId == 5))
                    {
                    ...
                    foreach(var claim in claims.Where(i => i.StatusId == 7))
                    {
                ...
                {
                    Logger.Log("ОШИБКА: Невозможно подключиться к серверу синхронизации заявок. Адрес, по которому была проведена попытка подключения: {0}", cf.Endpoint.Address.Uri);
                    throw;
                }
                catch(Exception ex)
                {
        ...
        public static void UpdateCustomersFromSite()
        {
            ChannelFactory<ISyncService> cf = new ChannelFactory<ISyncService>("SyncServiceEndpoint");
            var client = cf.CreateChannel();
            try
            {
                using(var context = new TonusEntities())
                {
                    ...
                    var newData = client.GetFromSite(context.LocalSettings.Single().DefaultDivisionId ?? context.Divisions.Single().Id, LastCustomerSync);

                    foreach(var customer in newData.Item1)
                    {
                        if(!context.Customers.Any(i => i.Id == customer.Id))
                        {
                            context.Customers.AddObject(customer);
                        }
                    }

                    foreach(var cn in newData.Item2)
                    {
                        if(!context.CustomerNotifications.Any(i => i.Id == cn.Id))
                        {
                    ...
                    LastCustomerSync = lastSyncTime;
                }
            }
            catch(EndpointNotFoundException)
            {
                Logger.Log("ОШИБКА: Невозможно подключиться к серверу синхронизации клиентов. Адрес, по которому была проведена попытка подключения: {0}", cf.Endpoint.Address.Uri);
                throw;
            }
            catch(Exception ex)
            {
