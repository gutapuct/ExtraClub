...
namespace ExtraClub.WorkflowService.CommandLine
{
    internal sealed class StreamCreator
    {
        public void Run(string[] args)
        {
            ...
            if (ver > 0)
            {
                ms = SyncCore.GetDataSetStream(cId, ver, (id, ver1) => GetLocalDelta(new ExtraEntities(), id, ver1), ref log, 0);
            }
            else
            {
        ...
        {
            using (var conn = new SqlConnection((((EntityConnection)new ExtraEntities().Connection).StoreConnection.ConnectionString)))
            {
                conn.Open();
