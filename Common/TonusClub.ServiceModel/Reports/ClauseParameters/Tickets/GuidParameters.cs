...
namespace TonusClub.ServiceModel.Reports.ClauseParameters.Tickets
{
    ...
    public class GuidParameterTi8 : ClauseGuidParameter<Ticket>
    {
        public override Func<ITonusService, Dictionary<object, string>> GetValuesFunction
        {
            get
            {
                return new Func<ITonusService, Dictionary<object, string>>(i => Enum.GetValues(typeof(TicketStatus)).Cast<TicketStatus>().ToDictionary(j => (object)(new Guid(0, (byte)j, 0, new byte[8])), j => Ticket.GetStatusName(j)));
            }
        }
