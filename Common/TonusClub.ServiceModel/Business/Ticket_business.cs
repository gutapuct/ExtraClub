...
#if BEAUTINIKA
            ExtraUnitsOut = UnitCharges.Sum(c => (int?)c.ExtraCharge) ?? 0;
#endif
        }
...
#if BEAUTINIKA
                && (UnitsAmount > 0 || SolariumMinutesLeft > 0 || ExtraUnitsLeft>0)
#else
...
#if BEAUTINIKA
                if (UnitsLeft <= 0 && SolariumMinutesLeft <= 0 && ExtraUnitsLeft <= 0) return TicketStatus.RunOut;
#else
...
#if BEAUTINIKA
        [DataMember]
        public decimal ExtraUnitsOut { get; set; }

        public decimal ExtraUnitsLeft
        {
            get
            {
                return ExtraUnitsAmount - ExtraUnitsOut;
            }
        }
#endif
        [DataMember]
