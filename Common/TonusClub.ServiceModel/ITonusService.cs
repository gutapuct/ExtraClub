...
namespace TonusClub.ServiceModel
{
    ...
    public interface ITonusService
    {
...
#if BEAUTINIKA
        [OperationContract]
        void PostLastTicketCorrectionBeau(Guid customerId, DateTime corrActivated, int corrAmount, int corrExtra, int corrGuest, decimal paidAmt, int solCorr);///b
#endif
        [OperationContract]
...
#if BEAUTINIKA
        [OperationContract]
        void PostTicketCorrection(Guid guid, int NewLength, int NewUnits, int newExtra, int NewGuest, int NewSolarium, int NewFreeze, string comment, DateTime? planInstDate);///b
#else
        [OperationContract]
        void PostTicketCorrection(Guid guid, int newLength, int newUnits, int newGuest, int newSolarium, int newFreeze, string comment, DateTime? planInstDate, string newComment);///t
#endif
        ...
        [OperationContract]
        [ApplyDataContractResolver]
        List<Room> GetRooms(Guid divisionId);///b

        [OperationContract]
        Guid PostRoom(Guid roomId, Guid divisionId, string roomName, Guid[] availTreatments);///b

        [OperationContract]
        [ApplyDataContractResolver]
        BeautinikaProposal GetBeautinikaProposal(Guid divisionId, Guid customerId, Guid ticketId, DateTime date, Guid mainTreatmentId, Guid mainRoomId, Guid mainEmployeeId, Guid extraTreatmentId, Guid extraRoomId, Guid extraEmployeeId);///b

        [OperationContract]
        [ApplyDataContractResolver]
        void PostBeautinikaProposal(Guid divId, BeautinikaProposal proposal);///b

        [OperationContract]
        [ApplyDataContractResolver]
        List<UpcomingEventInfo> GetUpcomingEvents(Guid divisionId);///b

        [OperationContract]
        void PostTreatmentConfigGoods(Guid treatmentConfigId, List<TreatmentConfigGood> amounts);///b

        [OperationContract]
        [ApplyDataContractResolver]
        List<TreatmentConfigGood> GetGoodsForTC(Guid treatmentConfigId);///b

        [OperationContract]
        [ApplyDataContractResolver]
        List<MaterialsReportLine> GetCurrentMaterialsAmount(Guid divisionId);///b

        [OperationContract]
        [ApplyDataContractResolver]
        List<Recommendation> GetRecommendationsForCustomer(Guid customerId);///b

        [OperationContract]
        [ApplyDataContractResolver]
        Guid PostRecommendation(Recommendation recommendation);///b

        [OperationContract]
        [ApplyDataContractResolver]
        List<CustomerTicketInfo> GetTicketsForCard(Guid companyId, string cardBarcode);///b


        [OperationContract]
        [ApplyDataContractResolver]
        void PostConsultation(Customer customer, DateTime dateTime, Guid employeeId, Guid roomId, Guid treatmentId);///b

        [OperationContract]
        [ApplyDataContractResolver]
        void PostMaterialCharges(Guid treatmentEventId, List<Tuple<Guid, decimal>> goods);///b

        [OperationContract]
        void PostTreatmentCosmetologist(Guid treatmentEventid, Guid employeeId);///b

        [OperationContract]
        void PostTreatmentConfigChange(Guid id, Guid configId, Guid treatmentId);///b
#endif
