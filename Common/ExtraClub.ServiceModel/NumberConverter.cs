...
{
    public enum Gender { Male, Female, Middle }
    ...
    public class RusNumber
    {
        ...
        public static string Str(int val, Gender gender, string one, string two, string five)
        {
            string[] frac20 =
            {
            ...
            foreach (XmlNode curr in section.ChildNodes)
            {
                if (curr.Name == "currency")
                {
                    ...
                    RusCurrency.Register(
                        ...
                        ((curr.Attributes["male"].InnerText == "1") ? Gender.Male : Gender.Female),
    ...
    public static class RusCurrency
    {
        ...
        static RusCurrency()
        {
            Register("RUR", Gender.Male, "рубль", "рубля", "рублей", "копейка", "копейки", "копеек");
            Register("EUR", Gender.Male, "euro", "euro", "euro", "eurocent", "eurocent", "eurocent");
            Register("USD", Gender.Male, "доллар", "доллара", "долларов", "цент", "цента", "центов");
            Register("TG", Gender.Male, "тенге", "тенге", "тенге", "тиын", "тиына", "тиынов");
            Register("SUM", Gender.Male, "сум", "сума", "сумов", "тийин", "тийина", "тийинов");
            ConfigurationSettings.GetConfig("currency-names");
        }

        public static void Register(string currency, Gender male,
        ...
        public static string Str(double val)
        {
            ...
            else if (Thread.CurrentThread.CurrentCulture.NumberFormat.CurrencySymbol == "тг.")
            {
                return Str(val, "TG");
            }
            else if (Thread.CurrentThread.CurrentCulture.NumberFormat.CurrencySymbol == "сум.")
            {
                return Str(val, "SUM");
            }
            else
        ...
        public static string Str(double val, Gender male,
            string seniorOne, string seniorTwo, string seniorFive,
            string juniorOne, string juniorTwo, string juniorFive)
        {
            bool minus = false;
            ...
            r.Insert(0, RusNumber.Str(n, Gender.Female, "тысяча", "тысячи", "тысяч"));
            n /= 1000;

            r.Insert(0, RusNumber.Str(n, Gender.Male, "миллион", "миллиона", "миллионов"));
            n /= 1000;

            r.Insert(0, RusNumber.Str(n, Gender.Male, "миллиард", "миллиарда", "миллиардов"));
            n /= 1000;

            r.Insert(0, RusNumber.Str(n, Gender.Male, "триллион", "триллиона", "триллионов"));
            n /= 1000;

            r.Insert(0, RusNumber.Str(n, Gender.Male, "триллиард", "триллиарда", "триллиардов"));
